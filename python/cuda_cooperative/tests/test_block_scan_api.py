...
def test_block_exclusive_scan_single_input_per_thread():
    # example-begin exclusive-scan-single-input-per-thread
    threads_in_block = 128

    # Specialize exclusive scan for a 1D block of 128 threads.  Each thread
    # owns a single integer item.
    block_exclusive_scan = cudax.block.exclusive_scan(
        dtype=numba.int32,
    ...
    # Link the exclusive scan to a CUDA kernel
    @cuda.jit(link=block_exclusive_scan.files)
    def kernel(data):
        thread_data = 1

        # Collectively compute the block-wide exclusive prefix scan.
        result = block_exclusive_scan(thread_data)

        # Copy the result back to the output.
        data[cuda.threadIdx.x] = result

    # example-end exclusive-scan-single-input-per-thread
